class_name ToymakerTurretEffect
...
func get_args(player_index: int) -> Array:
	...
	if is_burning:
		...
		return [str(spawn_cd), str(init_stats.burning_data.duration), str(init_stats.burning_data.damage), scaling_stats_text, tr(key.to_upper())]
	else:
		var scaling_stats_text = WeaponService.get_scaling_stats_icon_text(stats.scaling_stats)
		return [str(spawn_cd), str(init_stats.damage), scaling_stats_text, str(init_stats.nb_projectiles), str(init_stats.bounce), tr(key.to_upper())]
